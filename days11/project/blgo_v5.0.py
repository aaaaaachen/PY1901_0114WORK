...
# 收到的信息
def recive_message():
    if online.get("message") is not None:
...
def get_data():
    ...
    email1 = file['email1']
    # message1 = file['message1']
    # recyclebin = file['recyclebin']
    # accont = file['accont']
    print(file['users'].keys())

# 登录引擎函数
def engine():

    if not os.path.exists('E:/PY1901_0114WORK/days11/project/data/blog.dat.dat'):
        save_data()
    get_data()
    show_login()
