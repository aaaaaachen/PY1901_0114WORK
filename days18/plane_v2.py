...
class BossSprite(GameSprite):
    ...
    #重写父类开火方法
    def fire(self):
        bullet = BulletSprite(bullet_list[random.randint(7,9)], speed=-7, x=self.rect.centerx, y=self.rect.centery)
...
class Engine:
    ...
    def __init__(self):
        ...
        pygame.time.set_timer(SKILL_CREATE,20000)
    ...
    def update_sence(self):
        ...
        # 将当前得分和生命值显示到屏幕
        self.screen.blit(self.game_font.render('当前得分：%s' % self.sorce, True, [222, 211, 0]), [20, 20])
        self.screen.blit(self.game_font.render('生命值：%s' % self.life, True, [255, 222, 0]), [20, 50])
        # pygame显示更新
        pygame.display.update()
    ...
    def start(self):
        ...
        while True:
            ...
            # 判断是否升级
            if self.sorce > self.step_score_list[self.step-1] and len(self.boss_group) <= 0:
                self.sorce +=1
                self.step += 1
                # 当到第五关时，游戏不在升级
                if self.step >5:
        ...
        if len(event_list) > 0:
            ...
            for event in event_list:
                ...
                if event.type == SKILL_CREATE:
                    self.skill = Skill_Pack(skill_pack_list[random.randint(0,1)], speed=1)
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_r:
                        ...
                        if self.sorce > 20:
                            ...
                            # music = pygame.mixer.Sound("music/fire.wav")
                            # music.play()
                                self.hero.fire3()
                                self.sorce -= 20
    ...
    def check_collide(self):
        ...
        if len(accident)>0:
            ...
            self.sorce += 1
            if self.sorce >= 19 and self.sorce % 19 == 0:
                self.creat_boss()
